...
namespace console\modules\generator\generators\database_model;
...
class Generator extends \yii\gii\Generator
{
    ...
    public $queryClass;
    public $queryBaseClass = 'common\\AbstractActiveQuery';

    public $databaseModelClass = 'common\\BaseActiveRecord';
    public $databaseNs = 'common\\database';
...
//            [['modelClass'], 'validateModelClass', 'skipOnEmpty' => false],
            ...
            [['generateLabelsFromComments', 'useTablePrefix', 'useSchemaName', 'generateQuery', 'generateDatabase', 'generateModel', 'generateRelationsFromCurrentSchema'], 'boolean'],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            ...
            'generateDatabase' => 'Generate DatabaseModel',
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if ($this->generateDatabase) {
                $databaseModelPath = Yii::getAlias('@' . str_replace('\\', '/', $this->databaseNs)) . '/' . $modelClassName . '.php';
                $files[] = new CodeFile($databaseModelPath, $this->render('database.php', $params));
            }

            if ($this->generateModel) {
                $containModelPath = Yii::getAlias('@' . str_replace('\\', '/',
                            $this->containNs)) . '/' . $modelClassName . '.php';
                $params['ns'] = $this->containNs;
                $params['className'] = $modelClassName;
                $params['baseClass'] = "{$this->databaseNs}\\$modelClassName";
            ...
            if ($this->generateQuery) {
                $params['ns'] = $this->queryNs;
                $params['className'] = $queryClassName;
                $params['modelClassName'] = $modelClassName;

                $path = Yii::getAlias('@' . str_replace('\\', '/', $this->queryNs)) . '/' . $queryClassName . '.php';
        ...
        foreach ($relations as $name => $relation) {
            ...
            if ($generateQuery) {
                ...
                if (class_exists($queryClassRealName, true) && is_subclass_of($queryClassRealName, '\yii\db\BaseActiveRecord')) {
                    ...
                    $result[$name] = '\yii\db\ActiveQuery|' . (($this->databaseNs === $this->queryNs) ? $relation[1] : '\\' . $this->queryNs . '\\' . $relation[1]) . 'Query';
                }
            } else {
        ...
        foreach ($types as $type => $columns) {
            if ($type === 'required') {
                foreach ($columns as $column) {
                    $rules[Inflector::camelize("required_$column")] = "[['$column'], 'required']";
                }
                continue;
            }
    ...
    protected function generateRelationName($relations, $table, $key, $multiple)
    {
        ...
        if (!empty($key) && strcasecmp($key, 'fid')) {
            if (substr_compare($key, 'fid', -3, 3, true) === 0) {
                $key = rtrim(substr($key, 0, -3), '_');
            } elseif (substr_compare($key, 'fid_', 0, 4, true) === 0) {
                $key = ltrim(substr($key, 4, strlen($key)), '_');
            }
        }

        if (!empty($key) && strcasecmp($key, 'cid')) {
            if (substr_compare($key, 'cid', -3, 3, true) === 0) {
                $key = rtrim(substr($key, 0, -3), '_');
            } elseif (substr_compare($key, 'cid_', 0, 4, true) === 0) {
                $key = ltrim(substr($key, 4, strlen($key)), '_');
            }
        }
