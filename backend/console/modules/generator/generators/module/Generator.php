...
namespace console\modules\generator\generators\module;
...
EOD;
    ...
    {
        return $this->ns . "\\{$this->moduleID}\\controllers";
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $queryClassName = ($this->generateQuery)
                ? $this->generateQueryClassName($modelClassName) : false;
            $tableRelations = isset($relations[$tableName])
                ? $relations[$tableName] : [];
            ...
            $params = [
                ...
                'relationsClassHints' => $this->generateRelationsClassHints($tableRelations,
                    $this->generateQuery),
            ];

            $models[] = $params;

            $basePath = Yii::getAlias('@' . str_replace('\\', '/', $this->ns))
                . "/{$this->moduleID}";

            $databaseModelPath = "$basePath/models/read/{$modelClassName}Read.php";
            $files[] = new CodeFile(
                $databaseModelPath,
                $this->render('model-read.php', $params)
            );

            $databaseModelPath = "$basePath/models/write/{$modelClassName}Write.php";
            $files[] = new CodeFile(
                $databaseModelPath,
                $this->render('model-write.php', $params)
            );

            $databaseModelPath = "$basePath/controllers/{$modelClassName}Controller.php";
            $files[] = new CodeFile(
                $databaseModelPath,
                $this->render('controller.php', $params)
            );

//            $containModelPath = Yii::getAlias('@'.str_replace('\\', '/',
//                        $this->ns)).'/'.$modelClassName.'.php';
//            if (!file_exists($containModelPath)) {
//                $params['ns'] = $this->ns;
//                $params['className'] = $modelClassName;
//                $params['baseClass'] = "{$this->ns}\\$modelClassName";
//                $files[] = new CodeFile($containModelPath,
...
//            }
        ...
        $databaseModelPath = "$basePath/Module.php";
        $files[] = new CodeFile(
            $databaseModelPath,
    ...
    public function generateRelationsClassHints($relations, $generateQuery)
    {
        ...
        foreach ($relations as $name => $relation) {
            ...
            if ($generateQuery) {
                $queryClassRealName = '\\' . $this->ns . '\\' . $relation[1];
                if (class_exists($queryClassRealName, true)
                    ...
                    $result[$name] = '\yii\db\ActiveQuery|' . (($this->ns
                            === $this->ns) ? $relation[1]
                            : '\\' . $this->ns . '\\' . $relation[1]) . 'Query';
                }
            } else {
    ...
    {
        if ($this->tableNames !== null && mb_stripos($this->tableName, ',') === false) {
        ...
        if (strpos($this->tableName, '*') !== false) {
            ...
            foreach (explode(',', $this->tableName) as $tableName) {
