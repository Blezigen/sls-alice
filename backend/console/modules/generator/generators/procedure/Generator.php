...
namespace console\modules\generator\generators\procedure;
...
class Generator extends \yii\gii\Generator
{
    ...
    public $databaseModelClass = 'common\\BaseActiveRecord';
    public $databaseNs = 'console\\spocedures';
    public $containNs = 'common\\models';
    public $queryNs = 'common\\queries';
    ...
    {
        return 'Database model generator';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [
                ['baseClass'],
                'validateClass',
                'params' => ['extends' => ActiveRecord::className()]
            ],
            [
                ['queryBaseClass'],
                'validateClass',
    ...
    public function requiredTemplates()
    {
        ...
        return ['migration.phtml', 'delete.phtml'];
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            foreach ($tableSchema->columns as $column => $columnSchema) {
                if (in_array($column, [
                    "id",
                    "deleted_at",
                    "created_at",
                ...
                $selectTableArguments[] = "$column $columnSchema->dbType";
                $selectArguments[] = "$column";

                /** @var ColumnSchema $columnSchema */
                $arguments[] = "$column $columnSchema->dbType";
                $columns[] = "$column";
            }
            ...
            $filePrefix = 'm'.Carbon::now()->format('ymd_His');
            ...
            $params = [
                ...
                "name"                 => $tableName,
                "selectTableArguments" => $selectTableArguments,
                "selectArguments"      => $selectArguments,
            ];

            $filename = $params['sourceTableName'];

            $path = '@'.str_replace('\\', '/', $this->databaseNs).'/';
            ...
            if ($only === null || in_array("delete", $only)) {
                $files[] = new CodeFile(
                    Yii::getAlias($path."/{$filename}_delete.php"),
                    $this->render('delete.phtml', $params)
                );
            }

            if ($only === null || in_array("insert", $only)) {
                $files[] = new CodeFile(
                    Yii::getAlias($path."/{$filename}_insert.php"),
                    $this->render('insert.phtml', $params)
                );
            }

            if ($only === null || in_array("select", $only)) {
                $files[] = new CodeFile(
                    Yii::getAlias($path."/{$filename}_select.php"),
                    $this->render('select_multiple.phtml', $params)
                );
            }

            if ($only === null || in_array("select_id", $only)) {
                $files[] = new CodeFile(
                    Yii::getAlias($path."/{$filename}_select_byId.php"),
                    $this->render('select_single.phtml', $params)
                );
            }

            if ($only === null || in_array("update", $only)) {
                $files[] = new CodeFile(
                    Yii::getAlias($path."/{$filename}_update.php"),
                    $this->render('update.phtml', $params)
                );
            }
        }
    ...
    public function generateRelationsClassHints($relations, $generateQuery)
    {
        ...
        foreach ($relations as $name => $relation) {
            ...
            if ($generateQuery) {
                ...
                if (class_exists($queryClassRealName, true)
                    && is_subclass_of($queryClassRealName,
                        '\yii\db\BaseActiveRecord')
                ) {
                    /** @var \yii\db\ActiveQuery $activeQuery */
                    $activeQuery = $queryClassRealName::find();
                    $activeQueryClass = $activeQuery::className();
                    if (strpos($activeQueryClass, $this->databaseNs) === 0) {
                        $activeQueryClass
                            = StringHelper::basename($activeQueryClass);
                    }
                    $result[$name] = '\yii\db\ActiveQuery|'.$activeQueryClass;
                } else {
                    $result[$name] = '\yii\db\ActiveQuery|'.(($this->databaseNs
                            === $this->queryNs) ? $relation[1]
                            : '\\'.$this->queryNs.'\\'.$relation[1]).'Query';
                }
            } else {
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->foreignKeys as $refs) {
            ...
            $rules[]
                = "[['$attributes'], 'exist', 'skipOnError' => true, 'targetClass' => $refClassName::className(), 'targetAttribute' => [$targetAttributes]]";
        }
            ...
            foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
                ...
                foreach ($table->foreignKeys as $refs) {
                    ...
                    $relations[$refTableSchema->fullName][$relationName] = [
                        'return $this->'.($hasMany ? 'hasMany' : 'hasOne')
                ...
                if (($junctionFks = $this->checkJunctionTable($table))
                    === false
                ) {
                    continue;
                }

                $relations = $this->generateManyManyRelations($table,
                    $junctionFks, $relations);
            }
        }
        ...
        foreach ($schemaNames as $schemaName) {
            foreach ($db->schema->getTableSchemas($schemaName) as $table) {
                ...
                foreach ($table->foreignKeys as $refs) {
                    ...
                    $leftRelationName
                        = $this->generateRelationName($relationNames, $table,
                        $fks[0], false);
                    ...
                    $relationNames[$refTableSchema->fullName][$rightRelationName]
                        = true;
    ...
    protected function isHasManyRelation($table, $fks)
    {
        ...
        try {
            $uniqueKeys = array_merge($uniqueKeys,
                $this->getDbConnection()->getSchema()
                    ->findUniqueIndexes($table));
        } catch (NotSupportedException $e) {
            // ignore
        }
        foreach ($uniqueKeys as $uniqueKey) {
            if (count(array_diff(array_merge($uniqueKey, $fks),
                    array_intersect($uniqueKey, $fks))) === 0
            ) {
                return false;
            }
        }
    ...
    protected function checkJunctionTable($table)
    {
        ...
        for ($i = 0; $i < $foreignKeysCount; ++$i) {
            ...
            for ($j = $i + 1; $j < $foreignKeysCount; ++$j) {
                ...
                $fks = array_merge(array_keys($firstColumns),
    ...
    protected function generateRelationName($relations, $table, $key, $multiple)
    {
        ...
        if ($baseModel === null) {
            $baseClass = $this->databaseModelClass;
        ...
        if (strpos($this->tableName, '*') !== false) {
            ...
            foreach ($db->schema->getTableNames($schema) as $table) {
                if (preg_match($pattern, $table)) {
                    $tableNames[] = $schema === '' ? $table
                        : ($schema.'.'.$table);
                }
            }
        } elseif (($table = $db->getTableSchema($this->tableName, true))
            !== null
        ) {
...
//            $this->classNames[$this->tableName] = $this->modelClass;
    ...
    public function generateTableName($tableName)
    {
        ...
        if (preg_match("/^{$db->tablePrefix}(.*?)$/", $tableName, $matches)) {
            $tableName = '{{%'.$matches[1].'}}';
        } elseif (preg_match("/^(.*?){$db->tablePrefix}$/", $tableName,
            $matches)
        ) {
            $tableName = '{{'.$matches[1].'%}}';
        }
    ...
    protected function generateClassName($tableName, $useSchemaName = null)
    {
        ...
        if (($pos = strrpos($tableName, '.')) !== false) {
            if (($useSchemaName === null && $this->useSchemaName)
                || $useSchemaName
            ) {
        ...
        if ($this->standardizeCapitals) {
            $schemaName = ctype_upper(preg_replace('/[_-]/', '', $schemaName))
                ? strtolower($schemaName) : $schemaName;
            $className = ctype_upper(preg_replace('/[_-]/', '', $className))
                ? strtolower($className) : $className;
            $this->classNames[$fullTableName]
                = Inflector::camelize(Inflector::camel2words($schemaName
                .$className));
        } else {
            $this->classNames[$fullTableName] = Inflector::id2camel($schemaName
                .$className, '_');
        }

        if ($this->singularize) {
            $this->classNames[$fullTableName]
                = Inflector::singularize($this->classNames[$fullTableName]);
        }
